  ...
  try {
    ...
    for (const app of apps) {
      ...
      for (const tier of ['app4', 'app3', 'app2', 'app1'] as const) {
        ...
        const checkGroup = transformCheckGroup(app, tier, alertChannels);
        ...
        for (const checkCategory of tierConfigs) {
          ...
          if (checkCategory.api_check) {
            for (const check of checkCategory.api_check) {
              const apiCheck = transformApiCheck(app, tier, check, checkGroup);
          ...
          if (checkCategory.browser_check) {
            for (const check of checkCategory.browser_check) {
              const browserCheck = transformBrowserCheck(app, tier, check, checkGroup);
          ...
          if (checkCategory.multi_check) {
            for (const check of checkCategory.multi_check) {
              const multiCheck = transformMultiStepCheck(app, tier, check, checkGroup);
...
**Total Checks**: ${totalApiChecks + totalBrowserChecks + totalMultiChecks}
...
`,
    ...
    log(`  - Total checks: ${totalApiChecks + totalBrowserChecks + totalMultiChecks}`);
    console.log('');
    log('Next steps:');
